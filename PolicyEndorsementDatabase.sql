...
exec [PolicyEn].updatCustomer 'Samir','1988/08/10','M','Rajesh','Father','NS','Chennai','654789389','Monthly',100004;
...
--Procedure to select product details,customer details and display the details of this two tables by using comman policy number.
...
AS
		  ...
		  EXEC [PolicyEn].selectDetails 100004

--Procedure to select product details,customer details and display the details of this two tables by using customer name and customer dob.
CREATE PROCEDURE [PolicyEn].selectDetail
@cname VARCHAR(50),
@dob DATE
AS
    SELECT p.PolicyNumber,p1.ProdName,p1.ProductLine,c.CustName,c.CustAddress,c.CustPhoneNo,c.CustGender,c.DOB,c.Smoker,c.Hobbies,c.Nominee,c.Relation,c.Premium_payment_freq,c.Age
		  ...
		  INNER JOIN
		  [PolicyEn].[Customers] c ON (c.CustId=p.CustId) 
		   WHERE c.CustName=@cname AND c.DOB=@dob

EXEC [PolicyEn].selectDetail Sameer,'1996/01/20';
...
--Create table for storing document file path.
CREATE TABLE [PolicyEn].[Document]
(
policyno INT,
Docpath VARCHAR(MAX) 
)
DROP TABLE [PolicyEn].[Document]
 INSERT INTO [PolicyEn].[Document] VALUES(100003,'File.pdf') 

 INSERT INTO [PolicyEn].[Document] VALUES(100003,'File1.pdf') 
  INSERT INTO [PolicyEn].[Document] VALUES(100004,'File4.pdf') 
 SELECT * FROM [PolicyEn].[Document]

 --INSERT INTO [PolicyEn].[Document] (policyno, Docpath) SELECT policyno, Docpath FROM [PolicyEn].[Document] p1  WHERE NOT EXISTS(SELECT policyno,Docpath
   --                 FROM [PolicyEn].[Document] p2
     --              WHERE p1.policyno = p2.policyno);

--For practice to update status.
...
Begin
	...
	from [PolicyEn].[Customers]
	  ...
	  if @Ocadd != @cadd
	  Begin
		Insert into PolicyEn.[Status] values (@polno,@cadd,'approved') select @polno,@cadd from PolicyEn.[Status] where @polno=100003 ;
	  End
End 

EXEC [PolicyEn].updateCustomer
